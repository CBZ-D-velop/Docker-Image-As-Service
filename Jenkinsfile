pipeline {
    ...
    stages {
        ...
        stage("sonarqube-check") {
            ...
            steps {
                withCredentials([
                    ...
                    sh("#!/bin/bash\n sudo load-certificates --uri ${SONAR_HOST_URL#https://}")
                    sh("#!/bin/bash\n sonar-scanner")
                }
            }
        }
        ...
        stage("publish") {
            ...
            steps {
                withCredentials([
                    string(credentialsId: "JENKINS_CI_JENKINS_CI_DOCKER_HUB_CREDENTIALS", variable: "JENKINS_CI_JENKINS_CI_DOCKER_HUB_CREDENTIALS")
                    ]) {
                        dir("${TYPE}/${NAME}/latest") {
